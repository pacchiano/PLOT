...
class FeedforwardMultiLayerOneDim(torch.nn.Module):
    def __init__(self, input_size, hidden_sizes, activation_type = "sigmoid", device = torch.device("cpu")):
...
class TorchMultilayerBinaryLogisticRegression:
    def __init__(
        self,
        dim=None,
        alpha=1,
        ...
        self.network = FeedforwardMultiLayerOneDim(self.dim, self.representation_layer_sizes, 
            activation_type = activation_type, device = device)
